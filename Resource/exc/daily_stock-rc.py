...
def get_stock(stock_id):
    try:
        ...
        b = a[:2]
        if b == '停牌':
            stock['ds_status'] = 'stop'
        else:
            stock['ds_status'] = 'active'
        # 股票日期
        a = extract_date(a)
        if a == today:
            stock['ds_date'] = a
        ...
        if count > 5:
            ...
            response = requests.post('http://localhost:8088/insert_ds', headers=headers, data=json.dumps(stock))
            print(stock)
            time.sleep(1)
            return 'true'


def get_ds_stock(stock, x, y):
    i = x
    while i < y:
...
threads = []
t1 = threading.Thread(target=get_ds_stock, args=(stock_list, 0, 400))
threads.append(t1)
t1 = threading.Thread(target=get_ds_stock, args=(stock_list, 400, 800))
threads.append(t1)
t1 = threading.Thread(target=get_ds_stock, args=(stock_list, 800, 1200))
threads.append(t1)
t1 = threading.Thread(target=get_ds_stock, args=(stock_list, 1200, 1600))
threads.append(t1)
t1 = threading.Thread(target=get_ds_stock, args=(stock_list, 1600, 2000))
threads.append(t1)
t1 = threading.Thread(target=get_ds_stock, args=(stock_list, 2000, 2400))
threads.append(t1)
t1 = threading.Thread(target=get_ds_stock, args=(stock_list, 2400, len(stock_list)))
threads.append(t1)
